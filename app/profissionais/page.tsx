...
import { Search, Briefcase, User as UserIcon, ArrowRight } from "lucide-react";
...
export default function ProfissionaisVCA() {
  ...
  useEffect(() => {
    async function fetchProfessionals() {
      try {
        const res = await fetch(`/api/profissionais?q=${search}`);
        if (res.ok) {
      ...
      <div className="bg-background border-b pt-12 pb-16 px-4">
        <div className="container max-w-6xl mx-auto text-center space-y-4">
          ...
          <div className="max-w-xl mx-auto relative mt-8">
            ...
            <Input 
              placeholder="Busque por nome, profissão ou especialidade..." 
              className="h-14 pl-12 rounded-2xl border-2 focus-visible:ring-primary/20 bg-background shadow-xl shadow-primary/5"
              value={search}
