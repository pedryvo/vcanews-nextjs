...
export default function AdDetailPage() {
  ...
  if (loading) return <div className="h-[70vh] flex items-center justify-center"><Loader2 className="h-10 w-10 animate-spin text-primary" /></div>;
  if (!ad) return <div className="h-[70vh] flex items-center justify-center font-bold uppercase tracking-widest italic text-muted-foreground">Anúncio não encontrado</div>;
              ...
              {ad.images.length > 1 && (
                <div className="absolute inset-x-4 top-1/2 -translate-y-1/2 flex justify-between pointer-events-none">
                  <Button
                    variant="secondary"
                    size="icon"
                    className="h-12 w-12 rounded-2xl shadow-xl pointer-events-auto opacity-0 group-hover:opacity-100 transition-opacity"
